...
if __name__ == "__main__":
    print("Vision-Forge Report Tools Demo")
    print("=" * 50)

    table_html = render_table(
        headers=["模型", "mIoU", "FPS"],
        rows=[["SAM_ViT_B", "0.72", "45"], ["SAM_ViT_H", "0.78", "28"]]
    )
    print("Table:", table_html[:100], "...")

    chart_html = render_bar_chart(
        labels=["Epoch 1", "Epoch 2", "Epoch 3"],
        datasets=[{"label": "mIoU", "data": [0.65, 0.70, 0.72]}],
        title="训练曲线"
    )
    print("\nChart:", chart_html[:100], "...")

    metrics_html = render_metrics_card([
        {"label": "最终 mIoU", "value": "0.72", "trend": "up"},
        {"label": "训练时间", "value": "2h 15m", "trend": "neutral"}
    ])
    print("\nMetrics:", metrics_html[:100], "...")

    full_report = assemble_html(
        title="SAM_ViT_B 微调实验报告",
        sections=[
            {"type": "text", "title": "实验配置", "content": "本实验使用 SAM_ViT_B 模型进行微调..."},
            {"type": "html", "content": table_html}
        ],
        charts=[chart_html],
        mermaid_diagrams=[
            render_mermaid_diagram(
                "flowchart",
                [{"id": "A", "label": "数据加载"}, {"id": "B", "label": "特征提取"}, {"id": "C", "label": "注意力聚合"}],
                [{"from": "A", "to": "B"}, {"from": "B", "to": "C"}],
                "微调流程图"
            )
        ]
    )
    print(f"\nFull Report Length: {len(full_report)} chars")
    print("\nReport saved to memory (demo only)")
